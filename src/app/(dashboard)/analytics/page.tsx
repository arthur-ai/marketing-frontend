...
export default function AnalyticsPage() {
  ...
  const { data: dashboardData, isLoading: dashboardLoading } = useDashboardStats(userId)
  const { data: pipelineData, isLoading: pipelineLoading } = usePipelineStats(userId)
  const { data: contentData, isLoading: contentLoading } = useContentStats()
  const { data: trendsData, isLoading: trendsLoading } = useTrends(trendDays, userId)

  // Fetch all jobs to get unique user list for the filter dropdown
  const { data: jobsData } = useQuery({
    queryKey: ['results-jobs-for-users'],
    queryFn: () => api.listResultsJobs(1000),
    staleTime: 5 * 60 * 1000,
  })
  ...
  const uniqueUsers = (() => {
    ...
    for (const job of jobs) {
      ...
      const display =
        ...
        job.user_id
      seen.set(job.user_id, display)
    }
    return Array.from(seen.entries())
      .map(([id, display]) => ({ user_id: id, display }))
  ...
  return (
    <Box>
      ...
      {activeTab === 0 && (
        <>
          {selectedUserId && (
            <Box sx={{ mb: 2, p: 1.5, bgcolor: 'primary.50', borderRadius: 2, display: 'inline-flex', alignItems: 'center', gap: 1 }}>
              <PersonIcon fontSize="small" color="primary" />
              <Typography variant="body2" color="primary.main" fontWeight={600}>
                Showing data for: {uniqueUsers.find(u => u.user_id === selectedUserId)?.display || selectedUserId}
              </Typography>
            </Box>
          )}
                    ...
                    <LineChart data={trendChartData}>
                      ...
                      <Tooltip
                        formatter={(value, name) =>
                          name === 'Success Rate (%)' ? [`${value}%`, name] : [value, name]
                        }
                      />
                      <Legend />
                      <Line
                ...
                {contentLoading ? (
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={contentSourceData}>
                      ...
                      <Tooltip />
                      <Bar dataKey="value" fill="#2563eb" name="Content Items" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center', py: 4 }}>
