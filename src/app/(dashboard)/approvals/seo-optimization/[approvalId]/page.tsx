...
export default function SEOOptimizationApprovalPage() {
  ...
  useEffect(() => {
    if (approval && approval.pipeline_step !== 'seo_optimization') {
  ...
  if (isLoading) {
    return (
      <Container maxWidth="lg" sx={{ py: 4, textAlign: 'center' }}>
        <CircularProgress />
        <Typography sx={{ mt: 2 }}>Loading SEO Optimization approval...</Typography>
      </Container>
    )
  }
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            <Typography variant="h4" component="h1" sx={{ fontWeight: 700, mb: 1 }}>
              SEO Optimization Approval
            </Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 1 }}>
              <Chip label="SEO OPTIMIZATION" color="warning" sx={{ textTransform: 'uppercase' }} />
        ...
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          <Accordion expanded={expandedOutput} onChange={(_, isExpanded) => setExpandedOutput(isExpanded)}>
            ...
            <AccordionDetails>
              <Box>
                <StepEditor
                  stepName={approval.pipeline_step || 'seo_optimization'}
                ...
                {!editedData && (
                  <Box sx={{ mt: 2 }}>
                    <MarkdownSection content={formatApprovalOutput(approval.output_data, approval.pipeline_step || 'seo_optimization')} />
                  </Box>
                )}
              </Box>
            </AccordionDetails>
