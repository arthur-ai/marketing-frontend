...
export default function SEOOptimizationApprovalPage() {
  ...
  const actions = approval ? (
    <Button variant="outlined" onClick={() => router.push(getJobRoute('seo_optimization', approval.job_id))}>
      View in Job
    </Button>
  ) : undefined
  ...
  return (
    <LoadingErrorState
      ...
      loadingText="Loading SEO Optimization approval..."
    ...
    >
      {approval && (
        <Container maxWidth="lg" sx={{ py: 4 }}>
          <PageHeader
            title="SEO Optimization Approval"
            backPath="/approvals"
            backLabel="Back to Approvals"
            chips={chips}
            actions={actions}
              ...
              >
                <Box>
                  ...
                  {!editedData && (
                    <Box sx={{ mt: 2 }}>
                      <MarkdownSection
                        content={formatApprovalOutput(approval.output_data, approval.pipeline_step || 'seo_optimization')}
                      />
                    </Box>
                  )}
                </Box>
