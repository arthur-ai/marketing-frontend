...
export default function MarketingBriefApprovalPage() {
  ...
  useEffect(() => {
    if (approval && approval.pipeline_step !== 'marketing_brief') {
  ...
  const handleDecision = async (selectedDecision: 'approve' | 'reject' | 'modify' | 'rerun', editedDataOverride?: any) => {
    ...
    try {
      ...
      showSuccessToast(`Approval ${actionText}`, `Marketing Brief has been ${actionText}`)
  ...
  const actions = approval ? (
    <Button variant="outlined" onClick={() => router.push(getJobRoute('marketing_brief', approval.job_id))}>
      View in Job
    </Button>
  ) : undefined
  ...
  return (
    <LoadingErrorState
      ...
      loadingText="Loading Marketing Brief approval..."
    ...
    >
      {approval && (
        <Container maxWidth="lg" sx={{ py: 4 }}>
          <PageHeader
            title="Marketing Brief Approval"
            backPath="/approvals"
            backLabel="Back to Approvals"
            chips={chips}
            actions={actions}
          ...
          <Card sx={{ mb: 3 }}>
            <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
              ...
              >
                <StepEditor
                  stepName={approval.pipeline_step || 'marketing_brief'}
