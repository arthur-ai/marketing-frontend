...
interface ValidationStatusSectionProps {
  data: {
    ...
    author_validated?: boolean
    category_validated?: boolean
    tags_validated?: boolean
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          ...
          <Box>
            ...
            <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: 'wrap', gap: 1 }}>
              {getStatusChip(title_validated, 'Title')}
              {getStatusChip(content_validated, 'Content')}
              {getStatusChip(author_validated, 'Author')}
              {getStatusChip(category_validated, 'Category')}
              {getStatusChip(tags_validated, 'Tags')}
            </Stack>
          </Box>
          ...
          {is_valid && validation_issues.length === 0 && (
            <Alert severity="success" icon={<CheckCircle />}>
              <Typography variant="body2">
                All blog post fields have been validated successfully.
              </Typography>
            </Alert>
          )}
        </Stack>
